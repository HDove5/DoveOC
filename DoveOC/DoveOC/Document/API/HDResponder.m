...
// 定义了文本编辑事件的标准响应操作，如复制、剪切、粘贴和选择文本
@protocol UIResponderStandardEditActions <NSObject>
@optional
- (void)cut:(nullable id)sender API_AVAILABLE(ios(3.0));  // x
...
// 该方法只能在支持 Force Touch 功能的设备上使用，因此只有在 iOS 9.0 及更高版本中才可用。如果在不支持 Force Touch 的设备上调用该方法，将会导致崩溃
...
// 用于定义按键快捷键的行为。这些方法可以让响应者对象捕捉指定的按键事件并执行相应的操作
