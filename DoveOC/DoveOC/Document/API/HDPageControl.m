...
/// 您可以设置视图或控件是否允许连续的交互，以及如何响应用户的持续交互行为。如果该属性被设置为YES，则视图或控件将响应用户的所有连续交互行为，例如拖动或滑动手势。如果该属性被设置为NO，则视图或控件仅响应用户的瞬时交互行为，例如单击或触摸手势。需要注意的是，该属性只能适用于支持连续交互的视图或控件，例如UIScrollView或UISlider。如果该属性应用于不支持连续交互的视图或控件，则该属性可能不起作用
@property (nonatomic, assign) BOOL allowsContinuousInteraction API_AVAILABLE(ios(14.0));

/// The preferred image for indicators. Symbol images are recommended. Default is nil. 视图或控件的首选指示器图像
