...
typedef NS_ENUM(NSInteger, UIInterfaceOrientation) {
    UIInterfaceOrientationUnknown            = UIDeviceOrientationUnknown, // 竖直方向，Home 键在下方。
...
// 在使用后台刷新功能时，应用程序可以在后台下载新数据，并在下载完成后通知操作系统以更新应用程序界面
// 操作系统获取新数据操作的结果
typedef NS_ENUM(NSInteger, UIBackgroundRefreshStatus) {
    UIBackgroundRefreshStatusRestricted, // unavailable on this system due to device configuration; the user cannot enable the feature  表示获取到了新数据
...
typedef NS_ENUM(NSInteger, UIApplicationState) {
    UIApplicationStateActive,      // 处于前台运行状态
    UIApplicationStateInactive,    // 应用程序处于前台但未处于活动状态，应用程序收到一个来电或启动了另一个应用程序
    UIApplicationStateBackground   // 应用程序在后台运行，此时用户不能直接与应用程序进行交互
...
// UIView's userInteractionEnabled
{
...
// 应用程序处于活跃状态最上层的窗口，并通过该窗口对象获取应用程序的根视图控制器等信息
// 但是在支持多场景（Multiple Scene）的应用程序中，keyWindow 属性不能再被用于获取主窗口对象，因为一个应用程序可能会包含多个场景，每个场景都有自己的窗口层次结构
