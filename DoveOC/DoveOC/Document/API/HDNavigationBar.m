...
@optional
// 当新的导航项被推入导航栏时调用，返回一个BOOL值，指示是否应该继续推入该项
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPushItem:(UINavigationItem *)item; // called to push. return NO not to.
// 当新的导航项被成功推入导航栏时调用，允许委托进行任何相关的处理。
 ...
 */
// 表示导航栏的半透明属性，以及在iOS3.0及更高版本中可用的UIAppearance选择器
// 以控制导航栏是否半透明。如果该属性为YES，则导航栏会被设置为半透明，可以透过导航栏看到下方的内容；如果该属性为NO，则导航栏会被设置为不透明，无法看到下方的内容。

// 该属性是可通过UIAppearance选择器进行设置的，这意味着可以在应用程序中全局地配置导航栏的半透明属性。
//  启用半透明属性可能会影响性能
