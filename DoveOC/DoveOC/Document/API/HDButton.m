...
//UIImageSymbolConfiguration 用于配置和创建系统图标的 UIImage。它提供了许多选项，以控制生成的图像的大小、颜色、字体等。
...
/// If pointer effects are enabled for the button, this will return true when an effect is active.  // 悬停状态
@property (nonatomic, assign, readonly, getter = isHovered) BOOL hovered API_AVAILABLE(ios(15.0), tvos(15.0), watchos(8.0));

/// Returns true while the button is presenting a menu.某个对象或控件是否被按住（或拖动）的状态
...
/// Enables this button's built-in pointer interaction.  它与userInteractionEnabled属性类似，但是它专门用于指定视图是否响应指针事件
